...
遠端主機
---
* 先確認佈署主機的 kernel 版本能夠更換成 4.15.0-29-generic 或是到 Cuju 的 Github（https://github.com/Cuju-ft/Cuju/tree/support/kernel4.15 ）下載並安裝 ubuntu-18.04.1-live-server-amd64.iso (Ubuntu 18.04.1) 在更換 kernel 版本至 4.15.0-29-generic
...
* 建立 nfs server 
    ...
    加入下面那行
    ---
    "要當作 nfs 資料夾的絕對路徑" *(rw,no_root_squash,no_subtree_check)
    * example: /home/[your username]/nfsfolder *(rw,no_root_squash,no_subtree_check) 
...
* 更改 cuju.yml 內容
    ...
    nfs_server_ip_address: "nfs server 的 ip"
    nfs_server_folder_path: "nfs server 上要被 mount 的資料夾路徑"
    ```
...
* mount the nfs folder
    ```
    - name: mount local nfsfolder to remote /mnt/nfs
      become: yes
      mount:        # This module controls active and configured mount points in /etc/fstab .
        path: /mnt/nfs
        src: "{{ nfs_server_ip_address }}:{{ nfs_server_folder_path }}"
        fstype: nfs
        state: mounted
    ---
    $ sudo mount -t nfs [nfs_server_ip_address]:[nfs_server_folder_path] /mnt/nfs
    ```
