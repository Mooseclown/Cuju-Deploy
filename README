...
* 設定和啟動 nfs server
    ...
    - name: add line to /etc/exports
      ...
      lineinfile:
        path: /etc/exports
        regexp: "^/home/{{ install_machine_username }}/nfsfolder *(rw,no_root_squash,no_subtree_check)"
        line: "/home/{{ install_machine_username }}/nfsfolder *(rw,no_root_squash,no_subtree_check)"
    ---
    Insert this line in /etc/exports to add your NFS folder:

     /home/[install_machine_username]/nfsfolder *(rw,no_root_squash,no_subtree_check) 
...
* mount the nfs folder
    ...
    - name: mount remote nfsfolder to remote /mnt/nfs
      ...
      mount:        # This module controls active and configured mount points in /etc/fstab .
        path: /mnt/nfs
        src: "{{ install_machine_ip_address }}:/home/{{install_machine_username }}/nfsfolder"
        fstype: nfs
        state: mounted
    ---
    $ sudo mount -t nfs [install_machine_ip_address]:/home/[install_machine_username]/nfsfolder /mnt/nfs
    ```
